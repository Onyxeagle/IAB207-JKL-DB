...
def search():
    if (request.args['search'] and request.args['search'] != ""):
        ...
        events = db.session.scalars(db.select(Events).where(Events.name.like(query) or db.select(Events).where(Events.description).like(query) or db.select(Events).where(Events.genre).like(query)))
        print(events)
